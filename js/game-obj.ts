...
//   isLive = true;
  ...
  animation(gameWidth, gameHeight) {
    var _this = this,
      //实际的横向移动速度，左或者右
      _movepx =
        this.dom.offsetLeft > gameWidth / 2 ? -1 * this.movepx : this.movepx;
    //处理移动函数
    var process = function() {
      //敌机的x,y坐标
      var left = _this.dom.offsetLeft,
        top = _this.dom.offsetTop;
      //向右移动
      if (_movepx > 0) {
        left =
      ...
      if (
        top < gameHeight - _this.dom.clientHeight &&
        _this.isLive &&
        !isCrash
      ) {
        //继续移动
    ...
    for (var i = 1, l = this.bulletLevel; i <= l; i++) {
      ...
      bullet.checkBeat = function() {
        ...
        for (var i = 0, l = enemyList.length; i < l; i++) {
          ...
          if (
            Math.sqrt(
              Math.pow(e_left - b_left, 2) + Math.pow(e_top - b_top, 2)
            ) <=
            e_radius + b_radius
          ) {
            //敌机死亡
...
class Game {
  ...
  startFlyer() {
    ...
    //设置位置
    this.flyer.setPosition(
      this.gamePanel,
      this.gamePanel.clientWidth,
      this.gamePanel.clientHeight
    );
    //重写发弹函数
  ...
  startEnemy() {
    ...
    //随机出敌机的x坐标位置
    var randomX =
      Math.random() * (this.gamePanel.clientWidth / enemy.dom.clientWidth);
    ...
    enemy.OnCheckCrash = function() {
      ...
      if (
        Math.sqrt(
          Math.pow(_this.flyer.dom.offsetLeft - this.dom.offsetLeft, 2) +
            Math.pow(_this.flyer.dom.offsetTop - this.dom.offsetTop, 2)
        ) <=
          _this.flyer.dom.clientWidth / 2 + this.dom.clientWidth / 2 ||
        Math.sqrt(
          Math.pow(_this.flyer2.dom.offsetLeft - this.dom.offsetLeft, 2) +
